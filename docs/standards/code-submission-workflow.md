...
本规范使用 `$PROJECT_ROOT` 表示项目根目录。此变量的值为动态获取：
...
1. **环境变量**（最高优先级）
   ...
   export OCTO_HOME=/path/to/service-knowledge-manager
   ```
...
```bash
# ❌ 硬编码路径
cd /Users/gaowen/Code/service-knowledge-manager/backend

# ❌ 不检查路径直接执行
mvn clean install  # 可能不在项目根目录
```
